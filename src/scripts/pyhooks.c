...
PyObject *PyHooks_BuildInfo(PyObject *self, PyObject *args) {
  ...
  ITERATE_LIST(token, token_i) {
	...
	break;
      ...
      bprintf(buf, "%c%s%c", HOOK_STR_MARKER, PyUnicode_AsUTF8(var),
	      HOOK_STR_MARKER);
    }
	...
	break;
      ...
      bprintf(buf, "%ld", PyLong_AsLong(var));
    }
...
void PyHooks_Monitor(const char *type, const char *info) {
  ...
  if(list != NULL) {
    ...
    ITERATE_LIST(func, list_i) {
      ...
      PyObject *retval = PyObject_CallObject(func, arglist);
      //PyObject *retval = PyEval_CallObjectWithKeywords(func, arglist, NULL);
      // check for an error:
      if(retval == NULL)
	log_pyerr("Error running Python hook %s", type);
...
static struct PyModuleDef hooks_moduledef = {
    PyModuleDef_HEAD_INIT,
    "hooks",               // <-- should match what WAS in InitModule3
    "The python module for registering and running hooks.",
    -1,                     // ?
    NULL,                   // makePyMethods(pyhooks_methods)
    NULL,                   // m_slots
    NULL,                   // m_traverse
    NULL,                   // m_clear
    NULL                    // m_free
