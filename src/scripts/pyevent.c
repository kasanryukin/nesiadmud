...
PyObject *PyEvent_start(PyObject *self, PyObject *args, void *func) {
  ...
  if(PyOwner != Py_None && owner == NULL) {
    PyErr_Format(PyExc_Exception, "Owner supplied does not exist in game");
    return NULL;
  }
...
PyObject *PyEvent_start_update(PyObject *self, PyObject *args) {
  PyErr_Format(PyExc_Exception, "start_update is deprecated. Use start_event.");
  return NULL;
...
static struct PyModuleDef event_moduledef = {
    PyModuleDef_HEAD_INIT,
    "event",               // <-- should match what WAS in InitModule3
    "The event module handles delayed function calls.", // <-- should match what WAS in InitModule3
    -1,                     // ?
    event_module_methods,   // Module methods
    NULL,                   // m_slots
    NULL,                   // m_traverse
    NULL,                   // m_clear
    NULL                    // m_free
