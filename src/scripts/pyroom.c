...
int PyRoom_init(PyRoom *self, PyObject *args, PyObject *kwds) {
  ...
  if(PyLong_Check(who)) {
  ...
  else if(PyUnicode_Check(who)) {
    ROOM_DATA *room = NULL;
    if( (room = worldGetRoom(gameworld, get_fullkey_relative(PyUnicode_AsUTF8(who), get_script_locale()))) != NULL) {
    ...
    // create a fresh room with the given key, and add it to the game
      ...
      room = newRoom();
      roomSetClass(room, get_fullkey_relative(PyUnicode_AsUTF8(who),
					      get_script_locale()));
...
static PyObject *PyRoom_richcompare(PyObject *obj1, PyObject *obj2, int op) {
    PyRoom *room1, *room2;


    if (op != Py_EQ && op != Py_NE) {
      log_string("Invalid comparison operator for rooms.");
      PyErr_SetString(PyExc_TypeError, "Invalid comparison operator for rooms.");
      Py_RETURN_FALSE;
    }
    ...
    if (!PyRoom_Check(obj1) || !PyRoom_Check(obj2)) {
        log_string("Both operands must be pyroom objects.");
        PyErr_SetString(PyExc_TypeError, "Both operands must be PyRoom objects");
        Py_RETURN_FALSE;
    }
...
int PyRoom_setname(PyRoom *self, PyObject *value, void *closure) {
  ...
  roomSetName(room, PyUnicode_AsUTF8(value));
  return 0;
}
...
int PyRoom_setdesc(PyRoom *self, PyObject *value, void *closure) {
  ...
  ROOM_DATA *room;
  PYROOM_CHECK_ROOM_EXISTS(self->uid, room);
  roomSetDesc(room, PyUnicode_AsUTF8(value));
  return 0;
}
...
int PyRoom_setterrain(PyRoom *self, PyObject *value, void *closure) {
  ...
  if(terrainGetNum(PyUnicode_AsUTF8(value)) == TERRAIN_NONE) {
  ...
  ROOM_DATA *room;
  PYROOM_CHECK_ROOM_EXISTS(self->uid, room);
  roomSetTerrain(room, terrainGetNum(PyUnicode_AsUTF8(value)));
  return 0;
}
...
int PyRoom_setbits(PyRoom *self, PyObject *value, void *closure) {
  ...
  bitSet(roomGetBits(room), PyUnicode_AsUTF8(value));
  return 0;
}
...
PyObject *PyRoom_get_exit_dir(PyObject *self, PyObject *args) {
  ...
  if(exit == NULL) {
    PyErr_Format(PyExc_Exception, "Tried to get direction of nonexistant "
...
PyObject *PyRoom_dig(PyRoom *self, PyObject *value) {
  ...
  if(PyUnicode_Check(py_dest))
    dest = PyUnicode_AsUTF8(py_dest); // get_fullkey_relative(PyUnicode_AsUTF8(py_dest),get_script_locale());
...
PyObject *PyRoom_setvar(PyRoom *self, PyObject *args) {  
  ...
  if(room != NULL) {
    if(PyLong_Check(val))
      roomSetInt(room, var, (int)PyLong_AsLong(val));
    else if(PyFloat_Check(val))
      roomSetDouble(room, var, PyFloat_AsDouble(val));
    else if(PyUnicode_Check(val))
      roomSetString(room, var, PyUnicode_AsUTF8(val));
    else {
      PyErr_Format(PyExc_TypeError,
...
PyTypeObject PyRoom_Type = {
    ...
    0,		               /* tp_traverse */
    0,		               /* tp_clear */
    (richcmpfunc) PyRoom_richcompare,		               /* tp_richcompare */
    0,		               /* tp_weaklistoffset */
    0,		               /* tp_iter */
    0,		               /* tp_iternext */
    0,                         /* tp_methods */
...
static struct PyModuleDef room_moduledef = {
    ...
    "instancing rooms.",
    -1,                     // ?
    room_module_methods,    // Module methods
    NULL,                   // m_slots
    NULL,                   // m_traverse
    NULL,                   // m_clear
    NULL                    // m_free
...
PyInit_PyRoom(void) {
    ...
    PyRoom_addMethod("do_trigs", py_gen_do_trigs, METH_VARARGS | METH_KEYWORDS,
