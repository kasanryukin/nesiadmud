...
PyObject *mudsys_do_save(PyObject *self, PyObject *args) {
  ...
  if(PyChar_Check(val)) {
    ...
    if( (acct = PyAccount_AsAccount(val)) == NULL) {
      PyErr_Format(PyExc_Exception, "Tried to save nonexistant account.");
      return NULL;
    }
...
PyObject *mudsys_add_cmd(PyObject *self, PyObject *args) {
  ...
  if(PyFunction_Check(func)) {
    ...
    if(get_help(name,FALSE)==NULL && 
       ...
       PyUnicode_Check(((PyFunctionObject*)func)->func_doc)){
      BUFFER *buf = newBuffer(1);
      bufferCat(buf, PyUnicode_AsUTF8(((PyFunctionObject*)func)->func_doc));
...
PyObject *mudsys_handle_cmd_input(PyObject *self, PyObject *args) {
  ...
  if( (sock = PySocket_AsSocket(pysock)) == NULL) {
    PyErr_Format(PyExc_Exception, "Tried to run command for nonexistent socket.");
    return NULL;
  }
...
PyObject *mudsys_show_prompt(PyObject *self, PyObject *args) {
  ...
  if( (sock = PySocket_AsSocket(pysock)) == NULL) {
    PyErr_Format(PyExc_Exception, "Tried to show prompt to nonexistent socket.");
    return NULL;
  }
...
static struct PyModuleDef mudsys_moduledef = {
    PyModuleDef_HEAD_INIT,
    "mudsys",               // <-- should match what WAS in InitModule3
    "The mudsys module, for all MUD system utils.", // <-- should match what WAS in InitModule3
    -1,                     // ?
    NULL,                   // makePyMethods(pymudsys_methods), m_methods
    NULL,                   // m_slots
    NULL,                   // m_traverse
    NULL,                   // m_clear
    NULL                    // m_free
...
PyInit_PyMudSys(void) {
  ...
  PyObject *module = PyModule_Create(&mudsys_moduledef);
  if (module == NULL)
    return NULL;
