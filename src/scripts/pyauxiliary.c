...
PyObject *newPyAuxiliaryData(const char *keyword) {
  ...
  if(proto == NULL)
    ...
    PyObject *instance = PyObject_CallObject(proto, NULL);
...
PyObject *pyAuxiliaryDataRead(const char *keyword, STORAGE_SET *set) {
  ...
  if(proto == NULL)
    ...
    PyObject *instance = PyObject_CallObject(proto, args);
    if(instance == NULL)
...
static struct PyModuleDef auxiliary_moduledef = {
    PyModuleDef_HEAD_INIT,
    "auxiliary",               // <-- should match what WAS in InitModule3
    "The module for installing auxiliary data.", // <-- should match what WAS in InitModule3
    -1,                         // ?
    pyauxiliary_module_methods, // Module methods
    NULL,                   // m_slots
    NULL,                   // m_traverse
    NULL,                   // m_clear
    NULL                    // m_free
