...
PyObject *Py_CompileFile(char *fname) {
 ...
 // source = (read the file in)
  fclose(fl);
  PyObject *code = Py_CompileString(source,  fname, Py_file_input);
  free(source);
  return (PyObject *)code;
}
*/
// see https://github.com/NLnetLabs/unbound/commit/e0d426ebb10653a78bf5c4053198f6ac19bfcd3e

PyObject *Py_CompileFile(char *fname) {
    // Open the file for reading
    FILE *fl = fopen(fname, "r");
    if (fl == NULL) {
        // If the file cannot be opened, set the appropriate Python error and return NULL
        PyErr_SetFromErrno(PyExc_IOError);
        return NULL;
    }

    // Determine the size of the file
    fseek(fl, 0, SEEK_END); // Move the file pointer to the end of the file
    long fsize = ftell(fl); // Get the current position of the file pointer (file size)
    fseek(fl, 0, SEEK_SET); // Reset the file pointer to the beginning of the file

    // Allocate memory to hold the file content
    char *source = (char *)malloc(fsize + 1); // Allocate memory for the file content string
    if (source == NULL) {
        // If memory allocation fails, close the file, set the memory error, and return NULL
        fclose(fl);
    ...
    if (fread(source, 1, fsize, fl) != (size_t)fsize) {
        // If reading the file fails, close the file, free the allocated memory,
        // set the I/O error, and return NULL
        fclose(fl);
        free(source);
        PyErr_SetString(PyExc_IOError, "Failed to read file");
        return NULL;
    }
    source[fsize] = '\0'; // Null-terminate the string

    // Close the file now that we have read its content
    fclose(fl);

    // Compile the file content string into a code object using Py_CompileString
    PyObject *code_obj = Py_CompileString(source, fname, Py_file_input);
    free(source); // Free the allocated memory for the file content

    if (code_obj == NULL) {
        // If compilation fails, handle the error and return NULL
        // (compilation error will be set by Py_CompileString)
        return NULL;
    }
...
/*
    PyObject *typeObj = PyObject_Type(type);
    if (typeObj != NULL) {
      // Get the type name
      const char *typeName = ((PyTypeObject *)typeObj)->tp_name;
      log_string("Type type: %s\n", typeName);
      Py_DECREF(typeObj);
    }

    PyObject *typeObj2 = PyObject_Type(value);
    if (typeObj != NULL) {
      // Get the type name
      const char *typeName = ((PyTypeObject *)typeObj2)->tp_name;
      log_string("value type: %s\n", typeName);
      Py_DECREF(typeObj2);
    }
*/
    ...
    if (tracebackModule != NULL) {
        ...
        chrRetval = strdup((strRetval ? PyUnicode_AsUTF8(strRetval) :
			    "unknown error"));
...
char* getPythonTraceback(void) {
    ...
    if (tracebackModule != NULL) {
        ...
        if (PyExceptionInstance_Check(value)) {
            ...
            // Create a traceback for the string value
