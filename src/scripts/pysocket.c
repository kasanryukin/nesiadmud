...
PyObject *PySocket_richcompare(PyObject *obj1, PyObject *obj2, int op) {

    PySocket *socket1, *socket2;


    if (op != Py_EQ && op != Py_NE) {
      log_string("Invalid comparison operator for sockets.");
      PyErr_SetString(PyExc_TypeError, "Invalid comparison operator for sockets.");
      Py_RETURN_FALSE;
    }
    ...
    if (!PySocket_Check(obj1) || !PySocket_Check(obj2)) {
        log_string("Both operands must be PySocket objects.");
        PyErr_SetString(PyExc_TypeError, "Both operands must be PySocket objects");
        Py_RETURN_FALSE;
    }


    // Deal with cases where we know we're dealing with two objects.
    socket1 = (PySocket *)obj1;
    socket2 = (PySocket *)obj2;

    switch (op) {
        case Py_EQ:

            if (socket1->uid == socket2->uid)
                Py_RETURN_TRUE;
            else
                Py_RETURN_FALSE;
        case Py_NE:
...
int PySocket_set_outbound_text(PySocket *self, PyObject *value, void *closure) {
  ...
  if(sock == NULL)
    ...
    bufferCat(socketGetOutbound(sock), PyUnicode_AsUTF8(value));
    return 0;
  }
}
...
PyObject *PySocket_bust_prompt(PySocket *self, PyObject *closure) {
  SOCKET_DATA *sock = PySocket_AsSocket((PyObject *)self);
  if(sock == NULL) {
    PyErr_Format(PyExc_Exception, "Tried to bust prompt on nonexistent "
...
PyTypeObject PySocket_Type = {
    ...
    0,                         /*tp_getattr*/
    0,                         /*tp_setattr*/
    NULL,                      // (cmpfunc)PySocket_compare, tp_compare
    0,                         /*tp_repr*/
    0,                         /*tp_as_number*/
    0,                         /*tp_as_sequence*/
    0,                         /*tp_as_mapping*/
    ...
    0,		               /* tp_traverse */
    0,		               /* tp_clear */
    (richcmpfunc)PySocket_richcompare,		               /* tp_richcompare */
    0,		               /* tp_weaklistoffset */
    0,		               /* tp_iter */
    0,		               /* tp_iternext */
    0,                         /* tp_methods */
...
static struct PyModuleDef mudsock_moduledef = {
    ...
    "currently connected sockets.",
    -1,                     // ?
    socket_module_methods,    // Module methods
    NULL,                   // m_slots
    NULL,                   // m_traverse
    NULL,                   // m_clear
    NULL                    // m_free
...
PyInit_PySocket(void) {
    ...
    PySocket_addMethod("send", PySocket_send, METH_VARARGS | METH_KEYWORDS,
