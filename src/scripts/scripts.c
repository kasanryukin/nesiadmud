...
void testPythonModules() {
    // Call functions from your Python modules
    PyObject *pModule = PyImport_ImportModule("mud");
    if (pModule) {
        ...
        log_string("testPythonModules failed to import the mud module.");
        PyErr_Print();
    }
}


void log_name(PyObject *module) {

  if (module != NULL) {
    // Get the module's name object
    PyObject *moduleNameObj = PyModule_GetNameObject(module);

    if (moduleNameObj != NULL && PyUnicode_Check(moduleNameObj)) {
        // Convert the name object to a C string
        const char *moduleName = PyUnicode_AsUTF8(moduleNameObj);
        if (moduleName != NULL) {
            log_string("Module name: %s\n", moduleName);
        } else {
            // Failed to convert the name object to a string
            PyErr_Print();
        }
    } else {
        // Module name object is not a Unicode object
        PyErr_Print();
    }
...
void expand_dynamic_descs_dict(BUFFER *desc, PyObject *dict,const char *locale){
  ...
  for(i = 0; i < size; i++) {
	...
	bprintf(new_desc, "%s", PyUnicode_AsUTF8(retval));
      else if(PyLong_Check(retval))
	bprintf(new_desc, "%ld", PyLong_AsLong(retval));
