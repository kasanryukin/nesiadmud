...
static PyObject *PyExit_richcompare(PyObject *obj1, PyObject *obj2, int op) {

    if (op != Py_EQ && op != Py_NE) {
      log_string("Invalid comparison operator for exits.");
      PyErr_SetString(PyExc_TypeError, "Invalid comparison operator for exits.");
      Py_RETURN_FALSE;
    }
    ...
    if (!PyExit_Check(obj1) || !PyExit_Check(obj2)) {
        log_string("Both operands must be PyExit objects.");
        PyErr_SetString(PyExc_TypeError, "Both operands must be PyExit objects");
        Py_RETURN_FALSE;
    }
...
int PyExit_setkey(PyObject *self, PyObject *value, void *closure) {
  ...
  if(value == NULL || value == Py_None)
    ...
    exitSetKey(ex, PyUnicode_AsUTF8(value));
...
int PyExit_setdest(PyObject *self, PyObject *value, void *closure) {
  ...
  if(value == NULL || value == Py_None) {
    PyErr_Format(PyExc_Exception, "Cannot delete an exit %d's destination. "
		 "delete the exit from its room instead.", exitGetUID(ex));
    return -1;
  }
  else if(PyUnicode_Check(value))
    exitSetTo(ex, PyUnicode_AsUTF8(value));
  else if(PyRoom_Check(value)) {
    ROOM_DATA *room = PyRoom_AsRoom(value);
    if(room != NULL)
      exitSetTo(ex, roomGetClass(room));
    else {
      PyErr_Format(PyExc_Exception, "Tried to set exit %d's destination to "
...
int PyExit_setspotdiff(PyObject *self, PyObject *value, void *closure) {
  ...
  if(value == NULL || value == Py_None)
    ...
    exitSetHidden(ex, PyLong_AsLong(value));
  else {
    PyErr_Format(PyExc_TypeError,
...
int PyExit_setpickdiff(PyObject *self, PyObject *value, void *closure) {
  ...
  if(value == NULL || value == Py_None)
    ...
    exitSetPickLev(ex, PyLong_AsLong(value));
  else {
    PyErr_Format(PyExc_TypeError,
...
int PyExit_setkeywords(PyObject *self, PyObject *value, void *closure) {
  ...
  if(value == NULL || value == Py_None)
    ...
    exitSetKeywords(ex, PyUnicode_AsUTF8(value));
  else {
    PyErr_Format(PyExc_TypeError,
...
int PyExit_setopposite(PyObject *self, PyObject *value, void *closure) {
  ...
  if(value == NULL || value == Py_None)
    ...
    exitSetOpposite(ex, PyUnicode_AsUTF8(value));
  else {
    PyErr_Format(PyExc_TypeError,
...
int PyExit_setleavemssg(PyObject *self, PyObject *value, void *closure) {
  ...
  if(value == NULL || value == Py_None)
    ...
    exitSetSpecLeave(ex, PyUnicode_AsUTF8(value));
  else {
    PyErr_Format(PyExc_TypeError,
...
int PyExit_setentermssg(PyObject *self, PyObject *value, void *closure) {
  ...
  if(value == NULL || value == Py_None)
    ...
    exitSetSpecEnter(ex, PyUnicode_AsUTF8(value));
  else {
    PyErr_Format(PyExc_TypeError,
...
int PyExit_setdesc(PyObject *self, PyObject *value, void *closure) {
  ...
  if(value == NULL || value == Py_None)
    ...
    exitSetDesc(ex, PyUnicode_AsUTF8(value));
  else {
    PyErr_Format(PyExc_TypeError,
...
int PyExit_setname(PyObject *self, PyObject *value, void *closure) {
  ...
  if(value == NULL || value == Py_None)
    ...
    exitSetName(ex, PyUnicode_AsUTF8(value));
  else {
    PyErr_Format(PyExc_TypeError,
...
PyObject *PyExit_makedoor(PyExit *self, PyObject *value) {
  ...
  const char  *key = NULL;
  if(ex == NULL) {
    PyErr_Format(PyExc_Exception, "Tried to edit nonexistant exit, %d.",
		 self->uid);
    return NULL;
  }
...
PyTypeObject PyExit_Type = {
    ...
    .tp_name = "exit.Exit",               // tp_name
    .tp_basicsize = sizeof(PyExit),       // tp_basicsize
    .tp_dealloc = (destructor)PyExit_dealloc,  // tp_dealloc
    .tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_BASETYPE, // tp_flags
    .tp_doc = "Python Exit object",       // tp_doc
    .tp_richcompare = (richcmpfunc)PyExit_richcompare, // tp_richcompare
    .tp_init = (initproc)PyExit_init,     // tp_init
    .tp_new = PyExit_new,                 // tp_new
};
...
PyTypeObject PyExit_Type = {
    ...
    0,		               // tp_traverse
    0,		               // tp_clear
    (richcmpfunc)PyExit_richcompare, // tp_richcompare
    0,		               // tp_weaklistoffset
    0,		               // tp_iter
    0,		               // tp_iternext
    0,                         // tp_methods
...
// Define the module
static struct PyModuleDef exit_moduledef = {
    PyModuleDef_HEAD_INIT,
    "exit",               // Module name
    "The exit module, for all MUD exit-related stuff.", // Module docstring
    -1,                     // Size of per-interpreter state of the module, or -1 if the module keeps state in global variables.
    exit_module_methods,    // Module methods
    NULL,                   // m_slots
    NULL,                   // m_traverse
    NULL,                   // m_clear
    NULL                    // m_free
