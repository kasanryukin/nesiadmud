...
void pyolc_do_to_proto(PyObject *to_proto, PyObject *data, BUFFER *buf) {
  if(to_proto != NULL) {
	...
	bufferCat(buf, PyUnicode_AsUTF8(ret));
      Py_XDECREF(ret);
  }
}
...
void pyitem_olc_menu(SOCKET_DATA *sock, PyObject *data) {
  char *str = PyUnicode_AsUTF8(PyObject_GetAttrString(data, "__item_type__"));
...
static struct PyModuleDef olc_moduledef = {
    ...
    "olc",               // <-- should match what WAS in InitModule3
    ...
    "while online.",
    -1,                     // ?
    NULL,                   // makePyMethods(pyolc_methods)
    NULL,                   // m_slots
    NULL,                   // m_traverse
    NULL,                   // m_clear
    NULL                    // m_free
...
PyInit_PyOLC(void) {
  ...
  PyObject *m = PyModule_Create(&olc_moduledef);
