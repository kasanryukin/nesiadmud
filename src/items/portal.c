...
int PyObj_setportaldest(PyObject *self, PyObject *value, void *closure) {
  OBJ_DATA *obj = PyObj_AsObj(self);
  if(obj == NULL) {
    PyErr_Format(PyExc_Exception, "Tried to set destination for "
  ...
  if(PyUnicode_Check(value))
    portalSetDest(obj, PyUnicode_AsUTF8(value));
...
int PyObj_setportalleavemssg(PyObject *self, PyObject *value, void *closure) {
  OBJ_DATA *obj = PyObj_AsObj(self);
  if(obj == NULL) {
    PyErr_Format(PyExc_Exception, "Tried to set leave message for "
		 "nonexistent portal, %d", PyObj_AsUid(self));
    return -1;
  }
  else if(!objIsType(obj, "portal")) {
  ...
  if(PyUnicode_Check(value))
    portalSetLeaveMssg(obj, PyUnicode_AsUTF8(value));
  else {
...
int PyObj_setportalentermssg(PyObject *self, PyObject *value, void *closure) {
  OBJ_DATA *obj = PyObj_AsObj(self);
  if(obj == NULL) {
    PyErr_Format(PyExc_Exception, "Tried to set enter message for "
		 "nonexistent portal, %d", PyObj_AsUid(self));
    return -1;
  }
  else if(!objIsType(obj, "portal")) {
  ...
  if(PyUnicode_Check(value))
    portalSetEnterMssg(obj, PyUnicode_AsUTF8(value));
  else {
