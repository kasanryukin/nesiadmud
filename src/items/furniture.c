...
int PyObj_setfurncapacity(PyObject *self, PyObject *value, void *closure) {
  ...
  furnitureSetCapacity(obj, PyLong_AsLong(value));
  return 0;
}
...
int PyObj_setfurntype(PyObject *self, PyObject *value, void *closure) {
  OBJ_DATA *obj = PyObj_AsObj(self);
  if(obj == NULL) {
    PyErr_Format(PyExc_Exception, "Tried to set furniture type for "
  ...
  else if(furnitureTypeGetNum(PyUnicode_AsUTF8(value)) == FURNITURE_NONE) {
    PyErr_Format(PyExc_TypeError, "Invalid furniture type, %s", 
		 PyUnicode_AsUTF8(value));
    return -1;
  }

  furnitureSetType(obj, furnitureTypeGetNum(PyUnicode_AsUTF8(value)));
  return 0;
}
